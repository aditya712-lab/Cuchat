      ...
      <div className='channelbar-container'>
        <div className='serach-box'>
          <div className='profile-container' onClick={ToggleSidebar}>
                <img src={"../profiles/" + (user.profilePicture ? user.profilePicture : "default.jpg")} className='profile-image-top'></img>
  ...
  if(group===true){
    ...
    return (
      <div className='contact' onClick={onClick}>
          <img src={"../profiles/" + (conversation?.groupPicture ? conversation?.groupPicture : "default.jpg")} className='profile-image' ></img>
    ...
    return (
        ...
        <div className='contact' onClick={onClick}>
            <img src={"../profiles/" + user?.profilePicture} className={'profile-image ' + (isOnline?'online-image': 'offline-image')} ></img>
            <div className='contact-info'>
              <h1 className='contact-name'>{user?.username}</h1>
              {isTyping ? <small className='message-text green-text'><b>Typing...</b></small>
              : messages.length===0 ? "": <small className='message-text'><b>{(lastMessage.sender === currentUser._id) ? "You" : user?.username}</b> : {lastMessage.text}</small>
              }
            </div>
        </div>
    );
